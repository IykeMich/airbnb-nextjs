...
interface InputProps {
    id: string; label:string; type?:string; disabled?: boolean; formatPrice?: boolean;
    required?: boolean; register: UseFormRegister<FieldValues>; errors: FieldErrors
}
...
const Input: React.FC<InputProps> = ({
    ...
    return ( 
        <div className="w-full relative">
            {formatPrice && (
                <BiDollar size={24} className="text-neutral-700 absolute
                top-5 left-2" />
            )}
            <input 
            ...
            <label className={` absolute text-md duration-150 transform -translate-y-3 top-5 z-10 
                origin-[10] ${formatPrice ? 'left-9' : 'left-4'} peer-placeholder-shown:scale-100
